...
<div class="wrap">
	...
	<div id="commonInfo<?php if(isset($_POST['ftp_upload'])) echo 'Ftp'; ?>">		
		<form id="batchedit" method="post" enctype="multipart/form-data" action="<?php $manageMenu = new PhotoQ_Util_ManageAdminMenuLocation(); echo $manageMenu->getPageName(); ?>">
		<?php 
		...
		<div class="main info_group">
			...
			<div class="info_unit"><label><?php _e('Tags (separate multiple tags with commas: cats, pet food, dogs)', 'PhotoQ'); ?>:</label><br /><input type="text" name="tags_input" class="tags-input" id="tags-input" size="50" value="<?php echo $this->_oc->getValue('qPostDefaultTags') ?>" /></div>
			<?php endif; ?>
...
<?php if(!isset($_POST['ftp_upload'])): ?>
	<form action="<?php $manageMenu = new PhotoQ_Util_ManageAdminMenuLocation(); echo $manageMenu->getPageName(); ?>" method="post" enctype="multipart/form-data">
		<?php 
			if ( function_exists('wp_nonce_field')){
